...
> [!CAUTION]
> This document describes the schema-12 data model and intended contracts of WIP snapshot `d49e621`; it is not evidence that every runtime path enforces those contracts. The 2026-07-22 review found deviations in writer exclusion, database-derived path containment, hard-link publication, job recovery/backfill isolation, post-copy projection invalidation, read-only audit connections, and complete recovery. Do not infer live readiness from a table or status definition. See [docs/SAFETY_HOLD.md](docs/SAFETY_HOLD.md), [docs/FINDINGS_REGISTER.md](docs/FINDINGS_REGISTER.md), and [docs/ACTION_PRIORITY_MATRIX.md](docs/ACTION_PRIORITY_MATRIX.md).
...
> [!WARNING]
> The rows below represent the intended persisted contract, not proof of safe runtime publication/recovery. Current hard-link publication can leave a writable `.partial` alias to a canonical inode; several running jobs can be stranded; API writers bypass the maintenance lock; and reviewed-copy completion does not transactionally schedule all downstream preparation. See F01–F06 and F23–F33 in `docs/FINDINGS_REGISTER.md`.
...
> [!WARNING]
> The Stage 11 description is historical/intended. The current coordinator uses a moving keyset inventory, can undercount retries, and can miss concurrent lower-sorting assets; `preprocess --backfill` can claim unrelated job kinds; recovery is not universal; and the release audit is not provably read-only or a disaster-recovery audit. Do not resume it on the live vault.
...
`media-vault migrate --vault ...` is the only schema-upgrade entry point. It attempts to hold `state/active-writer.lock`, validate the input manifest, check backup space, create/validate a unique SQLite backup under `state/backups`, apply registered migrations transactionally, and check version, foreign keys, and integrity. A repeated run against the current schema is intended to be a validated no-op; transaction/interruption failures are intended to roll back and a post-commit validation failure to restore the backup. **Current deviation:** the lock is racy and review API writers do not participate, so migration cannot prove that it refuses every live writer; its capacity/publication/durability/restore evidence is also incomplete. Migration remains prohibited on the live vault until W01/W02/W09/W16 and the release gates are complete. The migration code itself is intended to operate on state metadata without reading or writing source/canonical media.
